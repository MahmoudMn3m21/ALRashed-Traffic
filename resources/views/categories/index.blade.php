...
@if ($categories->count() === 2)
    {{-- Full-page two-category design --}}
    <section class="categories-split-section d-flex flex-column flex-lg-row min-vh-100">
        @foreach ($categories as $category)
        <a href="{{ route('products.by_category', $category) }}" class="category-split-card text-decoration-none flex-grow-1 position-relative overflow-hidden">
            <div class="category-split-bg" style="background-image: url('{{ $category->image ? asset('storage/categories/' . $category->image) : asset('images/placeholder.jpg') }}');"></div>
            <div class="category-split-overlay"></div>
            <div class="category-split-content position-absolute top-50 start-50 translate-middle text-center text-white w-100 px-3">
                <h2 class="category-split-title mb-3">{{ $category->getName() }}</h2>
                <p class="category-split-count mb-4">{{ __('categories.products_count', ['count' => $category->products_count]) }}</p>
                <span class="btn btn-light btn-lg rounded-pill px-4">
                    {{ __('categories.browse_products') }} <i class="fas fa-arrow-{{ app()->getLocale() == 'ar' ? 'left' : 'right' }} ms-2"></i>
                </span>
            </div>
        </a>
        @endforeach
...
@push('styles')
<style>
.categories-split-section { min-height: 100vh; }
.category-split-card { min-height: 50vh; display: flex; align-items: stretch; }
.category-split-bg {
...
.category-split-content { z-index: 2; }
.category-split-title { font-size: clamp(1.5rem, 4vw, 2.5rem); font-weight: 700; text-shadow: 0 2px 10px rgba(0,0,0,0.5); }
.category-split-count { font-size: 1.1rem; opacity: 0.95; }
@media (max-width: 991.98px) {
    .categories-split-section { flex-direction: column; }
    .category-split-card { min-height: 50vh; }
}
</style>
@endpush
